...
flights %>% group_by(month) %>% summarize(avg.dep.delay = mean(dep_delay, na.rm = TRUE)) %>%
  plot()
...
# Hint: you'll have to perform a grouping operation before summarizing your data
flights %>% group_by(dest) %>% summarize(avg.arrival.delay = mean(arr_delay, na.rm = TRUE)) %>% arrange(desc(avg.arrival.delay))
